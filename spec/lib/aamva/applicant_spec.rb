describe Aamva::Applicant do
  ...
  describe '.from_proofer_applicant(applicant)' do
    it 'should create an AAMVA applicant with necessary proofer applcant data' do
      ...
      expect(aamva_applicant.address.line_1).to eq(proofer_applicant.address1)
      expect(aamva_applicant.address.line_2).to eq(proofer_applicant.address2)
      expect(aamva_applicant.address.city).to eq(proofer_applicant.city)
      expect(aamva_applicant.address.state).to eq(proofer_applicant.state)
      expect(aamva_applicant.address.zipcode).to eq(proofer_applicant.zipcode)
      expect(aamva_applicant.state_id_data.state_id_number).to eq(proofer_applicant.state_id_number)
